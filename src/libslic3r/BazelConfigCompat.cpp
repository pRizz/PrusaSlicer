...
const std::vector<std::string> &tilt_options() { return s_Bazel_sla_tilt_options; }
...
std::pair<GCodeThumbnailDefinitionsList, ThumbnailErrors> make_and_check_thumbnail_list(
    const std::string &thumbnails_string, const std::string_view def_ext
) {
...
std::string_view unsupported_bgcode_result(bgcode::core::EResult result) {
    ...
    switch (result) {
    case EResult::BlockNotFound: return "Block not found in Bazel proof-slice bgcode stub";
    case EResult::InvalidBinaryGCodeFile:
    default: return "Binary G-code parsing is deferred outside the Bazel proof slice";
    }
}
...
FileHeader::FileHeader()
    : magic(0), version(0), checksum_type(static_cast<uint16_t>(EChecksumType::CRC32)) {}

std::string_view translate_result(EResult result) { return unsupported_bgcode_result(result); }
...
EResult read_next_block_header(
    FILE &, const FileHeader &, BlockHeader &, EBlockType, std::byte *, size_t
) {
...
core::EResult SlicerMetadataBlock::read_data(
    FILE &, const core::FileHeader &, const core::BlockHeader &
) {
